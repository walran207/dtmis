...
$divisionOfficeId = (int)($_SESSION['office_id'] ?? 0);
$actedRows = [];
$actionTypes = ['Approved', 'Forwarded', 'Returned', 'Rerouted'];

try {
    $pdo = getDatabaseConnection();
    $actedRows = dashboard_fetch_division_staff_tracker_rows($pdo, $divisionOfficeId, $actionTypes, 100);
...
$pageSubtitle = 'Tracks documents handled by your division and section staff in real time.';
$searchPlaceholder = 'Search Tracking ID or subject';
$dashboardLivePath = app_url('actions/dashboard-live.php?scope=division_staff&actions=' . rawurlencode(implode(',', $actionTypes)));
...
$tableTitle = 'Division + Section Action Live Tracker';
$tableColumns = ['Tracking ID', 'Subject', 'Document Type (+ ARTA)', 'Current Holder', 'Last Action Time', 'Last Action by Team', 'Status', 'Quick Actions'];
