...
$actedRows = [];
$actionTypes = ['Created', 'Received', 'Forwarded', 'Returned'];
$metrics = [
    'pending_total' => 0,
...
foreach ($actedRows as $row) {
    ...
    if (str_contains($actionType, 'create')) {
        $createdCount++;
    }
...
$pageHeading = 'CENRO Audit Logs';
$pageSubtitle = 'Documents you created, received, and forwarded with current routing status.';
...
$kpiCards = [
    ['label' => 'Created by Me', 'icon' => 'blue', 'value' => (string)$createdCount],
    ['label' => 'Forwarded by Me', 'icon' => 'green', 'value' => (string)$forwardedCount],
...
$panels = [
    [
        'title' => 'CENRO Action Snapshot',
        'rows' => [
            ['label' => 'Created', 'value' => (string)$createdCount, 'width' => '34%'],
            ['label' => 'Forwarded', 'value' => (string)$forwardedCount, 'width' => '33%'],
    ...
    [
        ...
        'chips' => [
            'CENRO/PASU -> PENRO -> RECORDS-UNIT -> ORED',
            'ORED -> ARD TS/ARD MS -> Division/Section -> ORED',
            'ORED Sign -> RECORDS-UNIT Release -> Origin Office',
        ],
    ],
];

$tableTitle = 'CENRO Action Tracker';
